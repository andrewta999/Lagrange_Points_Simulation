...
function init(){
	...
	//initialize the obj
	obj = new Ball(6, 'aqua', m0);
	obj.pos2D = new Vector2D(canvas.width/2 + r/2, canvas.height/2 - r*Math.sqrt(3)/2);
	cm = (sun.pos2D.multiply(M).add(earth.pos2D.multiply(m))).multiply(1/(M + m));
	distance = obj.pos2D.subtract(cm);
	dist = distance.length();
	distance = new Vector2D(distance.y, -distance.x);
	distance = distance.multiply(1/distance.length());
	velo_obj = Math.sqrt(G*(M+m)/r)*dist/r;
	obj.velo2D = distance.multiply(velo_obj);
	console.log(obj.velo2D);
	obj.draw(context);
	...
	if(state == 0){
		for (let i = 0; i < 50; i++){
			...
			star.velo2D = distance.multiply(velo_obj);
...
function moveEarth(){
	...
	var force_obj_1 = Forces.gravity(G, M, m0, obj.pos2D.subtract(sun.pos2D));
	var force_obj_2 = Forces.gravity(G, m, m0, obj.pos2D.subtract(earth.pos2D));
	var total_force = force_obj_1.add(force_obj_2);
	obj.velo2D = obj.velo2D.addScaled(total_force.multiply(1/m0), dt);
	obj.draw(context);
